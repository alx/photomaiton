...
def save_ia_image(capture_path, index, prompt, annee):
    try:
        ...
        with Image.open(source_image_path) as source:
            ...
            #final_image.save(Path(capture_path, f"{index}_resize.jpg"))    
            ...
            print(annee)
            positive = str(prompt["positive"]) + str(annee)
            print(positive)
            payload = {"prompt": positive,"negative_prompt": prompt["negative"], "loras":json.dumps(prompt["loras"]), "file": img_str}
            available_urls = [
            ...
            if response.status_code == 200:
                ...
                filepath = Path(capture_path, f"{annee}_{index}.ia.jpg")
                ...
                with Image.open(filepath) as img:
                    ...
                    font = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", font_size)
                    draw = ImageDraw.Draw(img_final) 
                    draw.text((20, 20), str(annee),(255, 255, 255), font=font)
                    final_image_path = Path(capture_path, f"{annee}_{index}.ia.jpg")
    ...
    if fcn=="process":
        for annee in range(1930, 2025):
            ...
            for prompt_id, prompt_data in config["prompts"].items():
                if prompt_id == takethis:
                    for i in range(15):
                        print(i)
                        save_ia_image(CAPTURE_PATH, i, prompt_data, annee)
                    break
